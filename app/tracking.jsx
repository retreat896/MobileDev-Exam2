...
import AdvancedColorDetector from '../modules/AdvancedColorDetector';
import { colorFilter, colorFormat, getColorFormat, getSelectedColor, resetToDefaults, setSelectedColor, setTapPosition, tapPosition } from '../modules/tracking.global';
...
export default function VisionCameraExample() {
    ...
    // const videoFormats = device.formats.filter(format => format.photoHeight > 480 );
    // const format = useCameraFormat(device, [ { videoResolution: { width: 1280, height: 540 } }, { fps: 30 } ] );
    // device.formats.forEach((f) => { if (f.videoWidth <= 1280 && f.videoHeight >= 540) console.log(f); }); 
    // console.log();
    // console.log(format);
    ...
    }, [isFocused, intervalId, selectedColor_JS]);

    const frameProcessor = useSkiaFrameProcessor(useCallback((frame) => {
        "worklet";
    ...
    }, [minObjectSize, maxObjectSize, resize, maxObjectCount, pixelSelectRadius]));
